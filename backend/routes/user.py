...
from schemas import (
    ...
    PerfilContexto, PerfilContextoResponse, ProfileCompletenessResponse,
    SectorAffinityResponse, validate_password,
)
...
async def get_sector_affinity(
    ...
    try:
        ...
        result = await sb_execute(
            ...
            .select("sector_id, affinity_score, feedback_count")
            .eq("user_id", user_id)
        ...
        if "relation" in error_str and "does not exist" in error_str:
            logger.warning(
                "user_sector_affinity table not found (FEEDBACK-001 migration pending). "
                "Returning empty list for user %s",
